...
# Part 3. Comparing scRNA-mech pseudo, scRNA-enz pseudo, and bulk
...
#####################
# Remove gene rows that are lowly expr in samples - filter low counts <10 ct
flat_matrix_ALL <- flat_matrix_ALL[rowSums(flat_matrix_ALL > 10) != 0, ]
...
# Heatmap - use transformed counts
print('Heatmap of all transformed counts')
rld <- rlogTransformation(dds_ALL, blind=FALSE)
sig.dat <- assay(rld)[res_ALL$padj < THRES & !is.na(res_ALL$padj), ]
annC <- data.frame(condition=condition)
rownames(annC) <- colnames(sig.dat)
pheatmap(sig.dat, scale='row', fontsize_row=9,annotation_col = annC, show_rownames = FALSE, cluster_cols = TRUE, cluster_rows=TRUE)
```
