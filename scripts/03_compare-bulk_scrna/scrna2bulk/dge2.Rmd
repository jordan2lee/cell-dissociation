...
# Part 0. Are there patient specific effects?
...
patient <- c('7319', '7320','7319', '7320')
condition <- c('bulk','bulk', 'gt', 'gt')
meta_data <- as.data.frame(cbind(condition,patient) )
...
# DGE
#####################
dds_obj_ctrlBULK <- DESeqDataSetFromMatrix(countData = flat_matrix_ctrlBULK, colData = meta_data, design = ~ patient)
...
```{r, echo=FALSE}
print('here is a snippet of the top genes')
res_ctrlBULK[order(res_ctrlBULK$padj)[1:10],]
```




# Part 2. Comparing sc vs bulk
...
# Part 3. Comparing scEnz vs scMech
...
colnames(flat_matrix_enz) <- c("ensID",scenz_base_names)
# Combine mech and enz samples
flat_matrix_ALL <- merge(flat_matrix_enz, flat_matrix_mech, by ='ensID', all=TRUE) #merge by rowname
# Replace NA --> 0 
flat_matrix_ALL[is.na(flat_matrix_ALL)]=0
# Now that have combined properly fix so that rowname are genes
flat_matrix_ALL <- column_to_rownames(flat_matrix_ALL, var='ensID')
